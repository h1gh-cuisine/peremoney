...
export async function fetchMasterProjects() { return (await apiClient().get<ApiProject[]>('/cabinets')).map(mapMasterProject); }
export async function createMasterProject(input: { clientName: string; type: ProjectType; region: string; regionId: number; sphere: string; managerId: string; managerName: string; price: number; employeeLogin: string; clientLogin: string; idempotencyKey: string }) {
  ...
  const result = await apiClient().post<{ cabinet: ApiProject; credentials: null | { client: { login: string; password: string }; employee: { login: string; password: string } }; replayed?: boolean }>('/cabinets', {
    name: input.clientName, type: types[input.type], region: input.region, regionId: input.regionId, sphere: input.sphere,
  ...
  const project = mapMasterProject({ ...result.cabinet, contactsExported: 0, leadsExported: 0, sales: 0, ltv: 0,
...
export async function cloneMasterProject(sourceId: string, input: { name: string; type: ProjectType; price: number; managerId: string }) {
  const types: Record<ProjectType, ApiProject['type']> = { quals: 'VDL', package: 'PACKAGE', numbers: 'NUMBERS' };
  const result = await apiClient().post<{ cabinet: Record<string, unknown>; credentials: { client: { login: string; password: string }; employee: { login: string; password: string } } }>(`/cabinets/${sourceId}/clone`,
    { name: input.name, type: types[input.type], price: input.price, managerName: input.managerId });
  const cabinet = result.cabinet as Omit<ApiProject, 'contactsExported'|'leadsExported'|'sales'|'ltv'|'paymentsCount'|'avgCheck'|'clientLogin'|'employeeLogin'>;
  return { ...mapMasterProject({ ...cabinet, contactsExported: 0, leadsExported: 0, sales: 0, ltv: 0, paymentsCount: 0, avgCheck: 0,
