...
const SLOTS: Array<{ hour: number; task: ScheduledTask }> = [
  ...
  { hour: 11, task: ScheduledTask.CONTACTS_SYNC },
  { hour: 15, task: ScheduledTask.CONTACTS_SYNC },
...
export class SchedulerService implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  private dueSlots(now: Date) {
    ...
    return dates.flatMap((date) => SLOTS.map((slot) => ({
      task: slot.task,
